...
| 영역 | 실제 결과 | 주요 증거 |
|---|---|---|
| Rust 전체 | format PASS, Clippy `-D warnings` PASS, 48 tests PASS | workspace source/tests, `validation/RELEASE_CHECKS.txt` |
...
| Web Relay | native gRPC와 HLS URL 동시 접근, TS demux, bounded-history SSE PTS/ordinal correlation PASS | `scripts/run-web-relay-test.sh`, `validation/web_relay_four_pass_results.json` |
| Deployment | 6개 이미지 build, non-root/read-only/cap-drop/no-new-privileges runtime PASS; Compose config PASS | `docker/`, `compose.edge.yaml`, `compose.receiver.yaml` |
| Supply chain | 6개 CycloneDX SBOM 생성; Trivy fixable HIGH/CRITICAL 0 | `validation/security/` |
...
13. Web Relay는 optional consumer로 격리했고 기존 gRPC와 HLS/SSE를 같은 synthetic session에서 동시에 검증했다. pass-through SEI 노출과 평문 사내망 기본값은 운영 매뉴얼에 명시했다.
