...
Docker daemon은 WSL 안에서 직접 실행하며 Web Relay를 포함한 최종 이미지 6개를 빌드했다. Edge/Receiver/Web Relay는 Debian 12.15 runtime, RB-Y1은 Ubuntu 24.04 runtime이며 모든 배포 컨테이너 검증은 non-root, read-only root filesystem, `cap_drop: ALL`, `no-new-privileges` 조건에서 수행했다. Receiver의 강제 저용량 임계값은 `/readyz` HTTP 503과 `disk_pressure=low`를 반환했고, 같은 synthetic session의 native gRPC/HLS/SSE 동시 접근을 검증했다.
