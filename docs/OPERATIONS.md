...
4. Receiver, 선택적 Web Relay, Hardware Adapter, Edge 순서로 시작한다.
...
docker compose --profile web -f compose.receiver.yaml up -d web-relay
...
- Web Relay 손실: HLS/SSE만 중단; Receiver gRPC/ingest/raw/Dataset Builder 유지
...
USB controller bandwidth, encoder session, CPU/memory, virtual-output latency, encrypted SRT reconnect, metadata kbps, camera skew p95/max, disk throughput을 측정해 `MAX_CAMERAS`와 bitrate를 정한다. Relay 사용 시 camera/viewer별 network, HLS tmpfs, metadata history, first-frame latency도 측정한다. release 전 format/clippy/test, Python export loader scan, Docker builds/self-tests, Compose config, synthetic codec, Web Relay 통합, package/vendor scan과 `docs/audit/FINAL_RELEASE_AUDIT.md`를 모두 갱신한다.
